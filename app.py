...
def main(det_archs, reco_archs):
    ...
    # Wide mode
    st.set_page_config(page_title="OCR : Document Text Recognition", layout="wide")

    # Designing the interface
    st.title("OCR : Document Text Recognition")
    # For newline
    st.write("\n")
    # Instructions
    ...
    if st.sidebar.button("Analyze page"):
        if uploaded_file is None:
            ...
            with st.spinner("Analyzing..."):
                ...
                # Display JSON
                # df = pd.DataFrame.from_dict(page_export, orient='index')
                # df = df.transpose()
                # print(df['blocks'])
                st.markdown("\nHere are your analysis results in JSON format:")
                st.json(page_export)
                # st.dataframe(df['blocks'][0])
